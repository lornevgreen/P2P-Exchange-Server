...
Note - Email Id should be unique
User can update email and phone number only , condition will follow by the coins sn generated by raida server.
System will automaticaly check if user exist or not, then perform create or update user. Coin sn field will use as indentifier.
...
Get Anotehr User detials => Verified
Note - if user_sn will not parse then system will fetch existing record.
...
Note - As per current flow , If network not available then ad_owner_id will take.
Onwer_user_sn not mandetory, but make sure raida responsing
Desc not modetory
Apart from above two all fieds are modeoty to post new ad.
...
Note - One user always alloweed once to ad review.
Ad_id should valid.
